...
type Patient struct {
	...
	PatientPhone int `json:"Patient_Phone,omitempty"`
...
func (e PatientEdges) PrescriptionsOrErr() ([]*Prescription, error) {
	if e.loadedTypes[0] {
...
func (pa *Patient) assignValues(values ...interface{}) error {
	...
	if value, ok := values[1].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field Gender", values[1])
	} else if value.Valid {
		pa.Gender = value.String
	}
	if value, ok := values[2].(*sql.NullInt64); !ok {
		return fmt.Errorf("unexpected type %T for field Patient_Phone", values[2])
	} else if value.Valid {
		pa.PatientPhone = int(value.Int64)
	}
	return nil
}

// QueryPrescriptions queries the prescriptions edge of the Patient.
func (pa *Patient) QueryPrescriptions() *PrescriptionQuery {
	return (&PatientClient{config: pa.config}).QueryPrescriptions(pa)
}
...
func (pa *Patient) String() string {
	...
	builder.WriteString(", Patient_Phone=")
	builder.WriteString(fmt.Sprintf("%v", pa.PatientPhone))
	builder.WriteByte(')')
